...
export class DisplayLibrarianComponent implements OnInit {
  ...
  getLibrarian() {
    this.http.get<LoginResponse>(`${this.service.baseURL}/libraryms/library/admin/displayLibrarian`).subscribe(response => {
  ...
  deleteLibrarian(librarian) {
    this.http.delete(`${this.service.baseURL}/libraryms/library/admin/deleteLibrarian/${librarian.id}`).subscribe(resData => {
      if (resData) {
