...
books: BookResponse;
  ...
  searchBook(requestForm: NgForm) {
    this.http.post<BookResponse>(`${this.service.baseURL}/libraryms/library/librarian/getBooks/`, requestForm.value)
    .subscribe(response => {
      if (response != null) {
      this.books = response;
  ...
  requestBook(book) {
    this.http.get(`${this.service.baseURL}/libraryms/library/student/book/requset/${book.bookId}`)
    .subscribe(response => {
      if (response) {
