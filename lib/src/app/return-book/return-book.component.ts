...
export class ReturnBookComponent implements OnInit {
  ...
  getBookInfo(bookInfo: NgForm) {
    this.http.get<BookTransactionResponse>(`${this.service.baseURL}/libraryms/library/librarian/getIssuedInfo/${bookInfo.value.id}`)
      .subscribe(response => {
        if (response != null) {
  ...
  returnBook(id) {
    this.http.delete(`${this.service.baseURL}/libraryms/library/librarian/returnBook/${id}`)
      .subscribe(response => {
        if (response) {
